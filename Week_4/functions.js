...
String.prototype.toTitleCase = function(){
  ...
  class Owner extends GeneralUse {
    // funtion to addMovie
    addMovie(ctitle, cgenre, cdescription, cdirector, cactors, cyear, cruntime, crating, cvotes, crevenue, cmetascore, cnum_in_stock, ){
    ctitle = ctitle.toTitleCase();
    let newMovie = {
    ...
    if (this.allMovieData.length != 0){ 
      ...
      let currMovie = this.allMovieData.filter(movie=>movie.title===ctitle)[0];
      if (typeof currMovie != 'undefined'){
        console.log( `Movie with title '${ctitle}' is already in database, refer to id : '${currID}'`);return};
      // update movie array
    ...
    removeUser(username, userVar){
      delete this.users[username]; userVar = undefined;}
  
    // get movies on rent
    get onRent() {
      return this.moviesOnRent;}
    ...
    rentMovie(movieTitle, quantity){
      ...
      if (currMovie.title == movieTitle && currMovie.num_in_stock >= quantity){
        if (movieTitle in rentedMovies){
          ...
          rentedMovies[movieTitle] += quantity;} else {
          ...
          if (this.username in this.moviesOnRent){
            ...
            if (movieTitle in this.moviesOnRent[this.username]){
              this.moviesOnRent[this.username][movieTitle] += quantity;} else {
              this.moviesOnRent[this.username]=Object.assign(this.moviesOnRent[this.username], newMovie);
            }} else {
              this.moviesOnRent[this.username] = newMovie;
            }return;}
        if (currMovie.title == movieTitle && currMovie.num_in_stock < quantity){
          console.log(`The movie '${movieTitle}', has only ${currMovie.num_in_stock} copies in stock currently!`); return;}
